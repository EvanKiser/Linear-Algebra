...
class Vector(object):
    ...
    def angle_between(self, v, radians=True):
        ...
        if radians == True:
            return math.acos(dotproduct/(self.magnitude() * v.magnitude()))

        else:
            return math.degrees(math.acos(dotproduct/(self.magnitude() * v.magnitude())))
    ...
    def is_parallel(self, v):
        ...
        if (self.angle_between(v, radians=False) == 0) or (self.angle_between(v, radians=False) == 180):
            return True
        else:
            return False
...
def main():
    vector1 = Vector(tuple([8.218, -9.341]))
    vector2 = Vector(tuple([-1.129, 2.111]))
    print(vector1.plus(vector2))

    vector3 = Vector(tuple([7.119, 8.215]))
    vector4 = Vector(tuple([-8.223, 0.878]))
    print(vector3.minus(vector4))

    vector5 = Vector(tuple([1.671, -1.012, -0.318]))
    print(vector5.times_scalar(7.41))

    vector6 = Vector(tuple([-0.221, 7.437]))
    ...
    vector9 = Vector(tuple([1.996, 3.108, - 4.554]))
    print(vector9.normalization())

    vector10 = Vector(tuple([7.887, 4.138]))
    vector11 = Vector(tuple([-8.802, 6.776]))
    print(vector10.dot_product(vector11))

    vector12 = Vector(tuple([-5.955, -4.904, -1.874]))
    vector13 = Vector(tuple([-4.496, -8.755, 7.103]))
    print(vector12.dot_product(vector13))

    vector14 = Vector(tuple([3.183, -7.627]))
    vector15 = Vector(tuple([-2.668, 5.319]))
    print(vector14.angle_between(vector15, radians=True))

    vector16 = Vector(tuple([7.35, 0.221, 5.188]))
    vector17 = Vector(tuple([2.751, 8.259, 3.985]))
    print(vector16.angle_between(vector17, radians=False))

    vector18 = Vector(tuple([-7.579, -7.88]))
    vector19 = Vector(tuple([22.737, 23.64]))
    print("Orthogonal: " + str(vector18.is_orthogonal(vector19)))
    print("Parallel: " + str(vector18.is_parallel(vector19)))

    vector20 = Vector(tuple([-2.029, 9.97, 4.172]))
    vector21 = Vector(tuple([-9.231, -6.639, -7.45]))
    print("Orthogonal: " + str(vector20.is_orthogonal(vector21)))
    print("Parallel: " + str(vector20.is_parallel(vector21)))

    vector22 = Vector(tuple([-2.328, -7.284, -1.214]))
    vector23 = Vector(tuple([-1.821, 1.072, -2.94]))
    print("Orthogonal: " + str(vector22.is_orthogonal(vector23)))
    print("Parallel: " + str(vector22.is_parallel(vector23)))

    vector24 = Vector(tuple([.118, 4.827]))
    vector25 = Vector(tuple([0, 0]))
    print("Orthogonal: " + str(vector24.is_orthogonal(vector25)))
    print("Parallel: " + str(vector24.is_parallel(vector25)))

    vector26 = Vector(tuple([3.039, 1.879]))
    vector27 = Vector(tuple([0.825, 2.036]))
    print(vector26.component_parallel_to(vector27))

    vector28 = Vector(tuple([-9.88, -3.264, -8.159]))
    vector29 = Vector(tuple([-2.155, -9.353, -9.473]))
    print(vector28.component_orthogonal_to(vector29))

    vector30 = Vector(tuple([3.009, -6.172, 3.692, -2.51]))
    vector31 = Vector(tuple([6.404, -9.144, 2.759, 8.718]))
    print(vector30.component_parallel_to(vector31))
    print(vector30.component_orthogonal_to(vector31))

    vector32 = Vector(tuple([8.462, 7.893, -8.187]))
    vector33 = Vector(tuple([6.984, -5.975, 4.778]))
    print(vector32.cross_product(vector33))

    vector34 = Vector(tuple([-8.987, -9.838, 5.031]))
    vector35 = Vector(tuple([-4.268, -1.861, -8.866]))
